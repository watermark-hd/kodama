...
+ (void)initialize {
    ...
    gJaStrings = [[NSDictionary alloc] initWithObjectsAndKeys:
        ...
        PWRJPStr("コダマ"), @"appName",
        PWRJPStr("コダマについて"), @"about",
        PWRJPStr("終了"), @"quit",
        PWRJPStr("言語"), @"languageMenu",
        PWRJPStr("画像を隠す"), @"hideImage",
        PWRJPStr("OK"), @"ok",
        PWRJPStr("curlを実行できません(同梱版が使用不可で、MacPorts/Tigerbrew版も見つかりません)"), @"curlNotFound",
    ...
    gEnStrings = [[NSDictionary alloc] initWithObjectsAndKeys:
        ...
        @"OK", @"ok",
        @"Cannot run curl (bundled copy unavailable and no MacPorts/Tigerbrew curl found)", @"curlNotFound",
